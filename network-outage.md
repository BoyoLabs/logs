...
| 18:24:32 | WiFi connection to AP `[REDACTED-BSSID-A]` lost |
| 18:24:33 | `wpa_supplicant` disconnected (reason=4: disassociated due to inactivity) |
| 18:24:40 | Attempting to authenticate with `[REDACTED-BSSID-B]` (5240 MHz) |
| 18:24:41 | Associated and connected to `[REDACTED-BSSID-B]` |
...
- `[REDACTED-BSSID-A]` → `[REDACTED-BSSID-B]`

These are two radios on the same router or mesh system (same manufacturer OUI). The adapter lost the connection to one radio and reconnected to the other. Reason code 4 ("disassociated due to inactivity") typically indicates the AP deauthenticated the client, possibly due to signal handoff logic.

## What It Was NOT
- Not DHCP lease exhaustion — machine held a valid lease (`[REDACTED-LOCAL-IP]`, valid_lft ~24h) throughout
